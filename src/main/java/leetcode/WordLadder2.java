...
public class WordLadder2 {
    ...
    private Map<String, Set<String>> nextStep = new HashMap<>();
    private HashSet<HashSet<String>> superSet = new HashSet<>();
    private List<List<String>> result = new ArrayList<>();
    private int minSolution = Integer.MAX_VALUE;

    private boolean couldBeNextStep(String a, String b) {
        int differences = 0;
    ...
    private void analyzeDictionary(String begin, List<String> words) {
        Set<String> forBeginWord = new HashSet<>();
        for (String word : words) {
            if (couldBeNextStep(begin, word)) {
                forBeginWord.add(word);
            }
        }
        nextStep.put(begin, forBeginWord);

        for (String word1 : words) {
            Set<String> nextSteps = new HashSet<>();
            for (String word2 : words) {
                if (!word1.equals(word2)) {
                    if (couldBeNextStep(word1, word2)) {
                        nextSteps.add(word2);
                    }
                }
            }
            nextStep.put(word1, nextSteps);
        }
    }

    private void findPath2(String begin, String end) {
        ...
        while (true) {
            ...
            String word = w.get(index);
            Set<String> next = nextStep.get(word);

            if (word.equals(end)) {
                int currentIndex = index;
                LinkedList<String> result1 = new LinkedList<>();
                while (currentIndex != -1) {
                    result1.addFirst(w.get(currentIndex));
                    currentIndex = parent.get(currentIndex);
                }
                if (min == -1) {
                    min = result1.size();
                    result.add(result1);
                } else if (min == result1.size()) {
                    result.add(result1);
            ...
            for (String wordl : next) {

                HashSet<String> set = new HashSet<>();
                set.add(wordl);
                int currentIndex = index;
                while (currentIndex!=-1) {
                    String ww = w.get(currentIndex);
                    set.add(ww);
                    currentIndex = parent.get(currentIndex);
                }
    ...
    private void findPath(String begin, String end, LinkedList<String> path) {
        if (!begin.equals(end)) {
            Set<String> nextSteps = nextStep.get(begin);
            for (String word : nextSteps) {
                if (!inUse.contains(word)) {
                    inUse.add(word);
                    path.add(word);
                    findPath(word, end, path);
                    path.pollLast();
                    inUse.remove(word);
                }
            }
            ...
            //solved
            if (minSolution > path.size()) {
                minSolution = path.size();
            }
    ...
    public List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList) {
        analyzeDictionary(beginWord, wordList);

        LinkedList linkedList = new LinkedList();
        linkedList.add(beginWord);
        //findPath(beginWord, endWord, linkedList);
        long time1 = System.currentTimeMillis();
        findPath2(beginWord, endWord);
    ...
    private void filterResults(List<List<String>> result) {
        Iterator<List<String>> iterator = result.iterator();
        while (iterator.hasNext()) {
            List<String> val = iterator.next();
            if (val.size() > minSolution) {
                iterator.remove();
            }
        }
    }
